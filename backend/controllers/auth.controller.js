...
// Generate an authentication JWT token valid for 7 days
const createToken = (userId) => jwt.sign({ id: userId }, process.env.JWT_SECRET, { expiresIn: '7d' });

// ─── 1. REGISTER CONTROLLER (POST /api/auth/register) ────────────────────────
const register = async (req, res) => {
  try {
    const { name, email, password } = req.body;
    if (!name || !email || !password)
...
// ─── 3. FORGOT PASSWORD CONTROLLER (POST /api/auth/forgot-password) ─────────
const forgotPassword = async (req, res) => {
  try {
    ...
    const user = await User.findOne({ email: email.toLowerCase().trim() });
    if (!user) {
      // Security abstract: Avoid confirming or denying if the account exists
      return res.status(200).json({ success: true, message: 'Verification token sent to your email!' });
    }

    // Generate a secure 6-character uppercase validation token
    const resetToken = crypto.randomBytes(3).toString('hex').toUpperCase();

    // Set expiration deadline context windows to 15 minutes
    user.resetPasswordToken = resetToken;
    user.resetPasswordExpires = Date.now() + 15 * 60 * 1000;
    await user.save({ validateBeforeSave: false });

    // Node terminal fall-back preview logs if an active mail transposer isn't bound yet
    console.log(`\n======================================================`);
    console.log(`RESET TOKEN FOR: ${email}`);
    console.log(`TOKEN: ${resetToken}`);
    console.log(`======================================================\n`);
...
// ─── 4. RESET PASSWORD CONTROLLER (POST /api/auth/reset-password/:token) ─────
const resetPassword = async (req, res) => {
  try {
    const { email, newPassword } = req.body;
    const { token } = req.params; // Securely reading out token parameters directly from matching express path mapping variables
    ...
    // Save password payload strings directly.
    // (Note: This is fully correct if you use a pre('save') middleware hashing hook inside your User model architecture)
    user.password = newPassword; 

    // Discard single-use lifecycle trackers out of document records completely
    user.resetPasswordToken = undefined;
    user.resetPasswordExpires = undefined;
    await user.save();

    return res.status(200).json({ success: true, message: 'Password recreated successfully!' });
  } catch (error) {
    console.error('Reset password error:', error);
    return res.status(500).json({ success: false, message: 'Internal server error.' });
  }
...
// Unified module map interface export layer
module.exports = { 
  register, 
  login, 
  forgotPassword, 
